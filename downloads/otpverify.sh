...
chmod og-rxw /var/motp 2>/dev/null || { echo "FAIL! Need write-access to /var/motp"; exit 17; }
...
while [ $I -lt 36 ] ; do # 3 minutes before and after
	...
	if [ "$OTP" = "$PASSWD" ] ; then
		touch /var/motp/cache/$OTP || { echo "FAIL! Need write-access to /var/motp" ; exit 17; }
		echo "ACCEPT"
